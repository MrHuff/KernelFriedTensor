...
def generate_job_params(directory='job_dir/'):
    ...
    PATH = ['electric_data/',
            'CCDS_data/',
            'traffic_data/']
    lags_base =  list(range(0, 25)) + list(range(7 * 24, 8 * 24)) if dataset in [0,2] else [0,1,2]
    base_dict={
        'PATH':PATH[dataset],
        'reg_para_a':0,
        'reg_para_b':0,
        ...
        'normalize_Y':True,
        'patience': 100,
        'periods':7 if dataset in [0,2] else 1,
    ...
    counter = 0
    rang = 7 if dataset in [0,2] else 1
    for W in [0.0]:
...
if __name__ == '__main__':
    generate_job_params('jobs_traffic_3/')