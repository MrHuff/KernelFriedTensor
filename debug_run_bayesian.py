...
if __name__ == '__main__': #forecasts tends to converge to constant value for some reason...
    ...
    base_dict = {
        'PATH': PATH[dataset],
        'reg_para_a':1e-2, #for VI dont screw this up #Seems that it becomes overregularized in the bayesian case... But what's causing it?!
        ...
        'latent_scale': False,
        'dual': True,
        'init_max': 1e-2, #fixes regularization issues... Trick for succesfull VI
        'bayesian': True,
        'multivariate': True,
        'mu_a': 0,
        'mu_b': 0.5,
        ...
        'delete_side_info': [0,1] if dataset==2 else None,#"[1,2],#[0],
        'special_mode': 0,
        ...
        'lambda_T_x_b': 0.11,#625.1, Try lower values actually for KFT! #Regularization term seems to blow up if "overtrained on entire set"
        'patience': 500,#100,
        'periods':3,#7, 1
    ...
    # base_dict = load_obj('job_2.pkl', 'jobs_traffic_baysian_WLR_3/')
    # base_dict['save_path'] =  'bayesian_test_run'
    # base_dict['init_max']=1e-3
