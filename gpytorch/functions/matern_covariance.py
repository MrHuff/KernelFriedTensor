...
class MaternCovariance(torch.autograd.Function):
    ...
    def forward(ctx, x1, x2, lengthscale, nu, dist_func):
        ...
        scaled_unitless_dist = dist_func(x1_, x2_).mul(math.sqrt(2 * nu))
        if nu == 0.5:
            ...
            exp_component = scaled_unitless_dist_.neg().exp()
            covar_mat = exp_component
            if needs_grad:
                d_output_d_input = scaled_unitless_dist.div(lengthscale).mul(exp_component)
        elif nu == 1.5:
            ...
            exp_component = scaled_unitless_dist.neg().exp()
            covar_mat = linear_term.mul(exp_component)
            if needs_grad:
                d_output_d_input = scaled_unitless_dist_.pow(2).div(lengthscale).mul(exp_component)
        elif nu == 2.5:
            # 3 kernel sized Tensors if no grad else 4
            linear_term = scaled_unitless_dist.clone().add_(1)
            quadratic_term = scaled_unitless_dist.clone().pow(2).div(3)
            exp_component = scaled_unitless_dist.neg().exp()
            if needs_grad:
                covar_mat = (linear_term + quadratic_term).mul(exp_component)
                d_output_d_input = linear_term.mul(quadratic_term).mul(exp_component).div(lengthscale)
            else:
                covar_mat = exp_component.mul(linear_term.add_(quadratic_term))
        if needs_grad:
