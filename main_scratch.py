...
if __name__ == '__main__':
    with warnings.catch_warnings(): #There are some autograd issues fyi, might wanna fix it sooner or later
        ...
        tensor_architecture = {0:{'ii':[0],'r_1':1,'n_list':[shape[0]],'r_2':10,'has_side_info':False,'init_scale':1e-1},
                               1: {'ii': [1], 'r_1': 10, 'n_list': [shape[1]], 'r_2': 10, 'has_side_info': True,'init_scale':1e-4}, #Magnitude of kernel sum
                               2: {'ii': [2], 'r_1': 10, 'n_list': [shape[2]], 'r_2': 1, 'has_side_info': False,'init_scale':1e-1 },
                               }
        ...
        other_configs={
            'reg_para_a':0, #Regularization term! Need to choose wisely
            'reg_para_b': 0,
            'batch_size_a': 1e0,
            'batch_size_b': 1e0,
            ...
            'task':'reg',
            'epochs': 10,
            'bayesian': False, #Mean field does not converge to something meaningful?!
            ...
            'sub_epoch_V':100,
            'sub_epoch_ls':100,
            'config':{'full_grad':True,'multivariate':True}
        }
        j = job_object(
